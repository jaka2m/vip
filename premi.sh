...
rm -rf /root/premi.sh > /dev/null 2>&1
if [ "${EUID}" -ne 0 ]; then
...
echo -e "         Auther : ${green}Geo  ${NC}${YELLOW}(${NC} ${green}tunnel ${NC}${YELLOW})${NC}"
echo -e "       © Recode By Geo VPN ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC 
...
function password_default() {
    ...
    CHATID="-1467883032"
    KEY="6946747820:AAG0LHxHvy-e40ZpOwH0TuBQJEKM-Zj7Amc"
    TIME="10"
...
wget -q -O /usr/local/sbin/quota "https://raw.githubusercontent.com/jaka2m/vip/refs/heads/main/limit/quota"
chmod +x /usr/local/sbin/quota
chmod + x /usr/local/sbin/quota
cd /usr/local/sbin/
sed -i 's/\r//' quota
cd
wget -q -O /usr/bin/limit-ip "https://raw.githubusercontent.com/jaka2m/vip/refs/heads/main/limit/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
cd
